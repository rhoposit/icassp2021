...
class Model(nn.Module) :
    ...
    def forward(self, x, samples):
        ...
        return self.overtone(x, code_x, global_spk), vq_pen.mean(), encoder_pen.mean(), entropy, vq_pen_spk.mean(), encoder_pen_spk.mean(), entropy_spk
    ...
    def forward_validate(self, x, A, deterministic=False, use_half=False, verbose=False):
        ...
        with torch.no_grad() :
            ...
            global_spk = discrete_spk.squeeze(1)
        self.train()
        return vq_pen.mean(), encoder_pen.mean(), entropy, vq_pen_spk.mean(), encoder_pen_spk.mean(), entropy_spk
    ...
    def do_train(self, paths, dataset, optimiser, epochs, batch_size, num_workers, step, train_sampler, device, lr=1e-4,  spk_lr=0.01, valid_index=[], use_half=False, do_clip=False):
        ...
        for e in range(epochs) :
            ...
            for i, (speaker, wave16) in enumerate(trn_loader) :
                ...
                p_cf, vq_pen, encoder_pen, entropy, vq_pen_spk, encoder_pen_spk, entropy_spk = self(x, translated)                
                p_c, p_f = p_cf
                ...
                weight_spk = 100
                loss = loss_c + loss_f + vq_pen + encoder_weight * encoder_pen + (vq_pen_spk + encoder_weight * encoder_pen_spk) * weight_spk
...
f'-- Gmax:{running_max_grad:#.1} {running_max_grad_name} -- Gmin:{running_min_grad:#.15} {running_min_grad_name} -- S:{speed:#.4} steps/sec -- Step: {k}k'
...
f'-- Lc={max_loss_c:#.4} -- Lf={max_loss_f:#.4} -- Lvq={max_loss_vq:#.4} -- Lvqc={max_loss_vqc:#.4} -- Lvqcw={max_loss_vqcw:#.4} -- LvqS={max_loss_vq_spk:#.4} -- LvqcS={max_loss_vqc_spk:#.4} -- LvqcwS={max_loss_vqcw_spk:#.4}')
            ...
            val_loss_c, val_loss_f, val_loss_vq, val_loss_vqc, val_entropy, val_loss_vq_spk, val_loss_vqc_spk, val_entropy_spk = self.validate(paths, step, dataset.path, valid_index, device)
            logger.status(f'Valid: -- Lc={val_loss_c:#.4} -- Lf={val_loss_f:#.4} -- Lvq={val_loss_vq:#.4} -- Lvqc={val_loss_vqc:#.4} -- LvqS={val_loss_vq_spk:#.4} -- LvqcS={val_loss_vqc_spk:#.4} ')
    ...
    def validate(self, paths, step, data_path, test_index, device, deterministic=False, use_half=False, verbose=False):
        ...
        for i, (speaker, wave16, path) in enumerate(test_loader) :
            ...
            vq_pen, encoder_pen, entropy, vq_pen_spk, encoder_pen_spk, entropy_spk = self.forward_validate(samples, translated)
            running_loss_c += 0
            running_loss_f += 0
        ...
        return running_loss_c/iters, running_loss_f/iters, running_loss_vq/iters, running_loss_vqc/iters, running_entropy/iters, running_loss_vq_spk/iters, running_loss_vqc_spk/iters, running_entropy_spk/iters
