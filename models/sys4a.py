...
from layers.speaker_classifier_angular import SpeakerClassifierAngularSoftmax
from layers.speaker_classifier_angular import AngleLoss
...
class Model(nn.Module) :
    def __init__(self, nspeakers, rnn_dims, fc_dims, global_decoder_cond_dims, upsample_factors, normalize_vq=False,
        ...
        self.tap = TemporalPooling_spk(128)
        self.speaker_classifier = SpeakerClassifierAngularSoftmax(128, self.nspeakers)
    ...
    def forward(self, x, samples):
        ...
        tap_output = self.tap(continuous_spk)
        # speaker classifier here
        emb = tap_output.permute(0, 2, 1).squeeze(2)
        spk_predict_cos = self.speaker_classifier(emb)
        spk_predict = self.speaker_classifier.predict(emb)
        discrete, vq_pen, encoder_pen, entropy, _ = self.vq(continuous.unsqueeze(2))
        discrete_spk, vq_pen_spk, encoder_pen_spk, entropy_spk, _ = self.vqspk(tap_output.unsqueeze(2))
        code_x = discrete.squeeze(2)
        discrete_spk = discrete_spk.squeeze(2)
        global_spk = discrete_spk.squeeze(1)
        return self.overtone(x, code_x, global_spk), vq_pen.mean(), encoder_pen.mean(), entropy, vq_pen_spk.mean(), encoder_pen_spk.mean(), entropy_spk, spk_predict_cos, spk_predict
    ...
    def generate(self, x, A, deterministic=False, use_half=False, verbose=False):
        ...
        with torch.no_grad() :
            ...
            tap_spk = self.tap(continuous_spk)
            discrete, vq_pen, encoder_pen, entropy, _ = self.vq(continuous.unsqueeze(2))
    ...
    def forward_validate(self, x, A, deterministic=False, use_half=False, verbose=False):
        ...
        self.eval()
        with torch.no_grad() :
            continuous = self.encoder(A)
            continuous_spk = self.encoder_spk(A)
            tap_spk = self.tap(continuous_spk)
            emb = tap_spk.permute(0, 2, 1).squeeze(2)
            spk_predict_cos = self.speaker_classifier(emb)
            spk_predict = self.speaker_classifier.predict(emb)
            discrete, vq_pen, encoder_pen, entropy, _ = self.vq(continuous.unsqueeze(2))
            discrete_spk, vq_pen_spk, encoder_pen_spk, entropy_spk, _ = self.vqspk(tap_spk.unsqueeze(2))
        self.train()
        return vq_pen.mean(), encoder_pen.mean(), entropy, vq_pen_spk.mean(), encoder_pen_spk.mean(), entropy_spk, spk_predict_cos, spk_predict
    ...
    def do_train(self, paths, dataset, optimiser, epochs, batch_size, num_workers, step, train_sampler, device, lr=1e-4,  spk_lr=0.01, valid_index=[], use_half=False, do_clip=False):
        ...
        spk_criterion_angle = AngleLoss().cuda()
        ...
        for e in range(epochs) :
            ...
            running_spk_class_ent = 0.
            running_spk_class_acc = 0.
            ...
            for i, (speaker, wave16) in enumerate(trn_loader) :
                ...
                p_cf, vq_pen, encoder_pen, entropy, vq_pen_spk, encoder_pen_spk, entropy_spk, spk_predict_cos, spk_predict = self(x, translated)

                spk_true = torch.max(speaker, 1)[1]
                spk_loss = spk_criterion_angle(spk_predict_cos, spk_true)
                entropy_spk_class = spk_loss
                accuracy_spk_class = 100.0 * (torch.argmax(spk_predict.data, 1) == spk_true.cuda(device)).sum() / float(spk_true.shape[0])  
                ...
                weight_spk = 100
                loss = loss_c + loss_f + vq_pen + encoder_weight * encoder_pen + (vq_pen_spk + encoder_weight * encoder_pen_spk) * weight_spk + spk_loss
                ...
                running_spk_class_ent += entropy_spk_class
                running_spk_class_acc += accuracy_spk_class
                ...
                avg_spk_class_ent = running_spk_class_ent / (i + 1)
                avg_spk_class_acc = running_spk_class_acc / (i + 1)
...
f'-- SpkClEnt={avg_spk_class_ent:#.4} -- SpkClAcc={avg_spk_class_acc:#.4} -- S:{speed:#.4} steps/sec -- Step: {k}k'
...
f'-- Lc={max_loss_c:#.4} -- Lf={max_loss_f:#.4} -- Lvq={max_loss_vq:#.4} -- Lvqc={max_loss_vqc:#.4} -- Lvqcw={max_loss_vqcw:#.4} -- LvqS={max_loss_vq_spk:#.4} -- LvqcS={max_loss_vqc_spk:#.4} -- LvqcwS={max_loss_vqcw_spk:#.4}')
            ...
            val_loss_c, val_loss_f, val_loss_vq, val_loss_vqc, val_entropy, val_loss_vq_spk, val_loss_vqc_spk, val_entropy_spk, val_loss_spkclass, val_acc_spkclass = self.validate(paths, step, dataset.path, valid_index, device)
            logger.status(f'Valid: -- Lc={val_loss_c:#.4} -- Lf={val_loss_f:#.4} -- Lvq={val_loss_vq:#.4} -- Lvqc={val_loss_vqc:#.4} -- LvqS={val_loss_vq_spk:#.4} -- LvqcS={val_loss_vqc_spk:#.4} -- Lspkcl={val_loss_spkclass:#.4} -- Aspkcl={val_acc_spkclass:#.4}')
    ...
    def validate(self, paths, step, data_path, test_index, device, deterministic=False, use_half=False, verbose=False):
        ...
        running_loss_vqc_spk = 0.
        running_entropy_spk = 0.
        running_loss_spkclass = 0.
        running_acc_spkclass = 0.
        ...
        for i, (speaker, wave16, path) in enumerate(test_loader) :
            ...
            vq_pen, encoder_pen, entropy, vq_pen_spk, encoder_pen_spk, entropy_spk, spk_predict_cos, spk_predict = self.forward_validate(samples, translated)
            running_loss_c += 0
            running_loss_f += 0
            ...
            spk_true = torch.max(speaker, 1)[1]
            spk_loss = spk_criterion_angle(spk_predict_cos, spk_true)
            spk_acc = 100.0 * (torch.argmax(spk_predict.data, 1) == spk_true.cuda(device)).sum() / float(spk_true.shape[0])  

            running_loss_spkclass += spk_loss
            running_acc_spkclass += spk_acc
            
        return running_loss_c/iters, running_loss_f/iters, running_loss_vq/iters, running_loss_vqc/iters, running_entropy/iters, running_loss_vq_spk/iters, running_loss_vqc_spk/iters, running_entropy_spk/iters, running_loss_spkclass/iters, running_acc_spkclass/iters
