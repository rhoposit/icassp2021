...
import models.sys2 as sys2_model
import models.sys3 as sys3_model
import models.sys4 as sys4_model
import models.sys5 as sys5_model

# Angular softmax
import models.sys4a as sys4a_model
import models.sys5a as sys5a_model
...
def get_dataset(dataset_type, config, model_type):
    if dataset_type == 'vctk_wavernn':
        data_path = config.multi_speaker_data_path
        with open(f'{data_path}/index.pkl', 'rb') as f:
            index = pickle.load(f)
        # hold out 10 files per speaker seen in training
        # plus also hold out all data for 10 entire speakers
        train_set = index[:-10]
        unseen_set = index[-10:]
        ...
        dataset = env.MultiWaveRNNDataset(train_index, data_path, 109)
        spk_path = None
        ...
        print("Test set speakers: ", len(test_index))
        print("Held out test set speakers: ", len(unseen_index))
        dataset = env.MultispeakerDataset2(train_index, data_path, 109)
        spk_path = None
    else:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # WaveRNN system 1
    if model_type == 'sys1':
        dataset_type = 'vctk_wavernn'
        data_path, spk_path, index, test_index, train_index, dataset = get_dataset(dataset_type,config, model_type)
        model_fn = sys1_model.Model(vctk_num_spk, rnn_dims=config.rnn_dims, fc_dims=config.fc_dims, pad=2, upsample_factors=config.upsample_factors, feat_dims=80, global_decoder_cond_dims=vctk_num_spk).cuda()

    # System 2 (used in warmups)
    elif model_type == 'sys2':
        dataset_type = 'vctk2'
        data_path, spk_path, index, test_index, train_index, dataset = get_dataset(dataset_type,config, model_type)
        model_fn = sys2_model.Model(rnn_dims=config.rnn_dims, fc_dims=config.fc_dims, global_decoder_cond_dims=vctk_num_spk,upsample_factors=config.upsample_factors, normalize_vq=True, noise_x=True, noise_y=True)

    # Warm Sys3 
    elif model_type == 'sys3':
        dataset_type = 'vctk'
        data_path, spk_path, index, test_index, train_index, dataset = get_dataset(dataset_type,config, model_type)
        # load a warmed up model
        warm = sys3_model.Model(rnn_dims=config.rnn_dims, fc_dims=config.fc_dims, global_decoder_cond_dims=dataset.num_speakers(),upsample_factors=config.upsample_factors, normalize_vq=True, noise_x=True, noise_y=True)
        model_fn = env.restore_warmup(warm, config.warmup_model)
        
    # Warm Sys4
    elif model_type == 'sys4':
        dataset_type = 'vctk'
        data_path, spk_path, index, test_index, train_index, dataset = get_dataset(dataset_type,config, model_type)
        # load a warmed up model
        warm = sys4_model.Model(nspeakers=dataset.num_speakers(),rnn_dims=config.rnn_dims, fc_dims=config.fc_dims, global_decoder_cond_dims=dataset.num_speakers(),upsample_factors=config.upsample_factors, normalize_vq=True, noise_x=True, noise_y=True)
        model_fn = env.restore_warmup(warm, config.warmup_model)
        
    # Warm Sys4a
    elif model_type == 'sys4a':
        dataset_type = 'vctk'
        data_path, spk_path, index, test_index, train_index, dataset = get_dataset(dataset_type,config, model_type)
        # load a warmed up model
        warm = sys4a_model.Model(nspeakers=dataset.num_speakers(),rnn_dims=config.rnn_dims, fc_dims=config.fc_dims, global_decoder_cond_dims=dataset.num_speakers(),upsample_factors=config.upsample_factors, normalize_vq=True, noise_x=True, noise_y=True)
        model_fn = env.restore_warmup(warm, config.warmup_model)

    # Warm Sys5
    elif model_type == 'sys5':
        dataset_type = 'vctk'
        data_path, spk_path, index, test_index, train_index, dataset = get_dataset(dataset_type,config, model_type)
        # load a warmed up model
    ...
    # Warm Sys5a
    elif model_type == 'sys5a':
        dataset_type = 'vctk'
        data_path, spk_path, index, test_index, train_index, dataset = get_dataset(dataset_type,config, model_type)
        # load a warmed up model
        warm = sys5a_model.Model(nspeakers=dataset.num_speakers(),rnn_dims=config.rnn_dims, fc_dims=config.fc_dims, global_decoder_cond_dims=dataset.num_speakers(),upsample_factors=config.upsample_factors, normalize_vq=True, noise_x=True, noise_y=True)
        model_fn = env.restore_warmup(warm, config.warmup_model)
